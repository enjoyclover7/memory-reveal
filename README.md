...
- READY 후 전체 카드 2초 공개 및 동시 뒤집기 힌트
...
- 보상 이미지 11장을 중복 없이 랜덤 선택하고 전체 감상 후 초기화

게임 시작 시 Supabase 서버가 세션을 발급하고, 마지막 카드를 맞춘 순간 서버가 경과 시간을 확정합니다. 결과창의 닉네임 입력 시간은 기록에 포함되지 않습니다. 브라우저에는 공개 가능한 `anon` 키만 포함하며 서비스 역할 키나 DB 비밀 키는 넣지 않습니다.
